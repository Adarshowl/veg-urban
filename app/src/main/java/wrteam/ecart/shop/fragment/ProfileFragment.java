...
import wrteam.ecart.shop.helper.FileUtils;
import wrteam.ecart.shop.helper.Session;
import wrteam.ecart.shop.helper.Utils;
import wrteam.ecart.shop.retrofit.RetrofitClient;
        ...
        if (resultCode == RESULT_OK) {
            if (requestCode == SELECT_FILE) {
                ...
                try {
                    ...
                    File actualImageFile = FileUtils.from(getContext(), data.getData());

                    Log.e(TAG, "compressing start");
                    ...
                    Log.e(TAG, "compressing end");

                    myImage = compressedImageFile;
                ...
                assert result != null;
//                filePath = result.getUriFilePath(activity, true); on date - 7 feb 2023
...
//                    .addHeader(Constant.AUTHORIZATION, "Bearer " + ApiConfig.createJWT("eKart", "eKart Authentication"))
    ...
    private void UpdateProfileRetrofit() {
        ...
        try {
            Call<ProfileUpdate> call = RetrofitClient
                    .getInstance()
                    .getApi()
                    .update_profile_pic(
                            RetrofitClient.to_request_body(Constant.AccessKeyVal),
                            RetrofitClient.to_request_body(session.getData(Constant.ID)),
                            RetrofitClient.to_request_body(Constant.UPLOAD_PROFILE),
                            RetrofitClient.image_to_part("profile", myImage)
                    );

            Log.e(TAG, "update_profile_pic()");

            call.enqueue(new Callback<ProfileUpdate>() {
                @Override
                public void onResponse(Call<ProfileUpdate> call, retrofit2.Response<ProfileUpdate> response) {

                    Log.e(TAG, "res->\n" + response.toString());

                    ProfileUpdate profileUpdate = response.body();
                    Log.e(TAG, "res->\n" + new Gson().toJson(profileUpdate));

                    if (profileUpdate.getError() == true) {

                        Toast.makeText(getContext(), profileUpdate.getMessage(), Toast.LENGTH_SHORT).show();
                        Log.e(TAG, profileUpdate.getMessage());
                        return;
                    } else {

                        session.setData(Constant.PROFILE, profileUpdate.getProfile());
